...
namespace Sistema.Presentacion
{
    public partial class FrmVentas : Form
    {
        ...
        private void txtCodigo_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                if (e.KeyCode == Keys.Enter)
                {
                    ...
                    {
                        MessageBox.Show(Tabla.Rows[0]["stock"].ToString());
                        //Agregar este articulo y su detalle
                        this.AgregarDetalle(Convert.ToInt32(Tabla.Rows[0][0]), Convert.ToString(Tabla.Rows[0][1]),
                            Convert.ToString(Tabla.Rows[0][2]), Convert.ToDecimal(Tabla.Rows[0][3]), Convert.ToInt32(Tabla.Rows[0][4]));
        ...
        private void AgregarDetalle(int IdArticulo, string Codigo, string Nombre, decimal Precio, int Stock)
        {
            bool Agregar = true;
        ...
        private void CalcularTotales()
        {
            ...
            else
            {
                foreach (DataRow item in DtDetalle.Rows)
                {
                    ...
                    if (item.Table.Columns.Contains("descuento"))
                    {
                        decimal.TryParse(item["descuento"].ToString(), out descuento);
                    }

                    
                    decimal importeConDescuento = (precio * cantidad) * (1 - (descuento / 100));
                    
                    Total += importeConDescuento;
                }
            }

            decimal impuesto = 0;
            decimal.TryParse(txtImpuestos.Text, out impuesto);
        ...
        private void bntVerArticulos_Click(object sender, EventArgs e)
        {
            ...
            frm.ShowDialog();
            this.AgregarDetalle(Variables.IdArticulo, Variables.Codigo, Variables.NombreArticulo, Variables.Precio, Variables.Stock);
        }
        ...
        private void btnInsertar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (txtIdCliente.Text == string.Empty || txtImpuestos.Text == string.Empty || txtNumeroComprobante.Text == string.Empty || DtDetalle.Rows.Count == 0)
                {
                    this.MensajeError("Faltan campos por completar");
                }
                else
                {
                    //MessageBox.Show("jdfjdjf");
                    //Insertar el articulo
                    respuesta = NIngreso.Insertar(Convert.ToInt32(txtIdCliente.Text), Variables.IdUsuario, cmbComprobante.Text, txtSerieComprobante.Text.Trim(),
